...
def plot_dataset_statistics(stats: Dict[str, Any], save_path: Optional[str] = None) -> None:
    ...
    axes[1, 1].bar(
        ...
        [stats["total_interactions"], stats["unique_users"] * stats["unique_items"]],
    )
...
def validate_data_consistency(data_dir: str) -> Dict[str, bool]:
    ...
    try:
        ...
        # Check dimensions
        results["matrix_dimensions"] = interaction_matrix.shape[0] == len(
            user_to_idx
        ) and interaction_matrix.shape[1] == len(item_to_idx)
