...
class UserInteractionDataset(Dataset):
    ...
    def __init__(self, interaction_matrix: csr_matrix, user_indices: Optional[List[int]] = None):
        ...
        self.user_indices = user_indices if user_indices is not None else list(range(interaction_matrix.shape[0]))
    ...
    def train_epoch(self, train_loader: DataLoader) -> Dict[str, float]:
        ...
        for batch_idx, batch_data in enumerate(pbar):
            ...
            if hasattr(self.model, 'compute_loss'):
                ...
                # For regular VAE
                loss, recon_loss, kl_loss = vae_loss_function(recon_x, x, mu, logvar, self.model.beta)
...
def train_hybrid_vae(data_dir: str,
                    embeddings_path: str,
                    output_dir: str,
                    latent_dim: int = 200,
    ...
    # Load training data
    interaction_matrix, train_df, val_df, mappings = load_training_data(data_dir)
    user_to_idx = mappings['user_to_idx']
    n_users, n_items = interaction_matrix.shape
    
    # Load item embeddings
    embeddings, item_to_idx, idx_to_item = load_embeddings(embeddings_path)
    
    # Validate embeddings match dataset
    assert item_to_idx is not None and len(item_to_idx) == n_items, f"Embeddings ({len(item_to_idx) if item_to_idx is not None else 'None'}) don't match dataset ({n_items})"
    ...
    train_dataset = UserInteractionDataset(interaction_matrix, train_user_indices)
    val_dataset = UserInteractionDataset(interaction_matrix, val_user_indices)
...
def main():
    ...
    parser.add_argument('--data', default=str(config.DATA_DIR), help='Directory containing processed dataset')
    parser.add_argument('--embeddings', default=config.EMBEDDINGS_FILE, help='Path to item embeddings')
    parser.add_argument('--output', default=str(config.MODEL_DIR), help='Output directory for trained model')
    parser.add_argument('--latent-dim', type=int, default=config.LATENT_DIM, help='Latent space dimension')
