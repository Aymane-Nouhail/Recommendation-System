...
def recall_at_k(recommended_items: np.ndarray, 
...
def ndcg_at_k(recommended_items: np.ndarray, 
              relevant_items: np.ndarray, 
              k: int) -> float:
    """
...
def hit_ratio_at_k(recommended_items: np.ndarray, 
                   relevant_items: np.ndarray, 
                   k: int) -> float:
    """
    ...
    def get_user_recommendations(self, 
        ...
        with torch.no_grad():
            ...
            user_vector = torch.FloatTensor(
                self.interaction_matrix[user_idx].toarray().flatten()
    ...
    def evaluate_dataset(self, 
        ...
        for k in k_values:
            avg_metrics[k] = {
                ...
                'hit_ratio': np.mean(all_metrics[k]['hit_ratio']) if all_metrics[k]['hit_ratio'] else 0.0
            }
...
def evaluate_recommendation_model(model_path: str,
                                 data_dir: str,
                                 embeddings_path: str,
    ...
    for k in k_values:
        if k in results:
            metrics = results[k]
            logger.info(f"@{k:2d}: Recall={metrics['recall']:.4f}, "
